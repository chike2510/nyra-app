...
.agent-hero{position:relative;overflow:hidden;background:linear-gradient(135deg,rgba(62,207,142,0.13),rgba(168,230,61,0.035) 58%,rgba(17,17,17,0.85));border:1px solid rgba(168,230,61,0.22);border-radius:16px;padding:22px;margin-bottom:16px}
.agent-hero::after{content:'';position:absolute;right:-70px;top:-85px;width:240px;height:240px;border-radius:50%;background:radial-gradient(circle,rgba(168,230,61,0.16),transparent 66%);pointer-events:none}
.agent-hero-top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;position:relative;z-index:1}
.agent-kicker{display:flex;align-items:center;gap:7px;font-size:11px;font-weight:700;color:var(--accent);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:8px}
.agent-title{font-family:'Space Grotesk',sans-serif;font-size:25px;line-height:1.1;letter-spacing:-0.035em;color:var(--t1);max-width:520px}
.agent-copy{font-size:13px;color:var(--t2);line-height:1.65;margin-top:9px;max-width:570px}
.agent-next{position:relative;z-index:1;display:flex;align-items:center;gap:12px;margin-top:18px;padding:13px 14px;background:rgba(8,8,8,0.42);border:1px solid rgba(255,255,255,0.08);border-radius:11px}
.agent-next-icon{width:34px;height:34px;border-radius:9px;background:var(--accent-dim);display:flex;align-items:center;justify-content:center;flex-shrink:0}
.agent-next-label{font-size:10px;font-weight:700;color:var(--t3);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:3px}
.agent-next-title{font-size:13px;color:var(--t1);font-weight:600}
.agent-next-sub{font-size:11px;color:var(--t2);margin-top:2px}
.agent-actions{position:relative;z-index:1;display:flex;gap:9px;margin-top:16px;flex-wrap:wrap}
.action-queue{margin-bottom:16px}
.queue-item{display:flex;align-items:center;gap:12px;padding:14px 16px;background:rgba(245,158,11,0.075);border:1px solid rgba(245,158,11,0.22);border-radius:12px;cursor:pointer;transition:all var(--dur) var(--ease)}
.queue-item:hover{background:rgba(245,158,11,0.12);transform:translateY(-1px)}
.queue-icon{width:36px;height:36px;border-radius:10px;background:rgba(245,158,11,0.12);display:flex;align-items:center;justify-content:center;flex-shrink:0}
.queue-eyebrow{font-size:10px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--warn);margin-bottom:3px}
.queue-title{font-size:13px;font-weight:650;color:var(--t1)}
.queue-copy{font-size:11px;color:var(--t2);margin-top:2px;line-height:1.45}
.decision-receipt{margin-top:11px;padding:13px;background:var(--s1);border:1px solid var(--border);border-radius:10px;animation:fadeUp 0.2s var(--ease) both}
.decision-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}
.decision-title{font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--t2)}
.decision-why{font-size:12px;color:var(--t1);line-height:1.55;margin-bottom:11px}
.decision-checks{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}
.decision-check{display:inline-flex;align-items:center;gap:4px;padding:4px 7px;border-radius:6px;background:var(--s2);border:1px solid var(--border);font-size:10px;color:var(--t2)}
.decision-actions{display:flex;gap:8px}
.permission-summary{background:linear-gradient(135deg,rgba(62,207,142,0.12),rgba(168,230,61,0.03));border:1px solid rgba(168,230,61,0.2);border-radius:14px;padding:18px;margin-bottom:14px}
.permission-summary h2{font-family:'Space Grotesk',sans-serif;font-size:17px;letter-spacing:-0.02em;color:var(--t1);margin-bottom:6px}
.permission-summary>p{font-size:12px;color:var(--t2);line-height:1.65;margin-bottom:14px}
.permission-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
.permission-pill{padding:10px;border-radius:9px;background:rgba(8,8,8,0.3);border:1px solid var(--border)}
.permission-pill strong{display:block;font-size:10px;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:5px}
.permission-pill span{font-size:11px;color:var(--t2);line-height:1.45;display:block}
.permission-pill.auto strong{color:var(--accent)}
.permission-pill.ask strong{color:var(--warn)}
.permission-pill.never strong{color:var(--err)}
.segmented{display:flex;gap:4px;padding:3px;background:var(--s3);border:1px solid var(--border);border-radius:9px}
.seg-btn{border:0;background:transparent;color:var(--t3);font-family:'Inter',sans-serif;font-size:10px;font-weight:700;padding:6px 8px;border-radius:6px;cursor:pointer;transition:all var(--dur)}
.seg-btn.auto.on{background:var(--accent-dim);color:var(--accent)}
.seg-btn.ask.on{background:rgba(245,158,11,0.1);color:var(--warn)}
.seg-btn.never.on{background:var(--err-dim);color:var(--err)}
@media(max-width:768px){.agent-hero{padding:18px}.agent-title{font-size:21px}.agent-hero-top{flex-direction:column}.agent-actions .btn{flex:1;justify-content:center}.agent-next{align-items:flex-start}.permission-grid{grid-template-columns:1fr;gap:7px}.permission-pill{display:flex;gap:8px;align-items:baseline}.permission-pill strong{margin:0;min-width:76px}.segmented{width:100%}.seg-btn{flex:1}.queue-item{align-items:flex-start}.decision-actions .btn{flex:1;justify-content:center}}
...
function DecisionReceipt({ tx, onOpenPolicies }) {
  const isBlocked = tx.status === "BLOCKED";
  const isEscalated = tx.status === "ESCALATED";
  const why = isBlocked
    ? (tx.reason || "This payment did not meet one of your rules.")
    : isEscalated
      ? (tx.reason || "This payment needs your confirmation before Nyra can proceed.")
      : "Nyra checked your payment rules and approved this transaction within your limits.";
  const checks = isBlocked
    ? ["Rule checked", "Action stopped"]
    : isEscalated
      ? ["New recipient", "Awaiting you"]
      : ["Within limit", "Trusted recipient", "Allowed time"];
  return (
    <div className="decision-receipt">
      <div className="decision-head">
        <span className="decision-title">Decision details</span>
        <Chip status={tx.status}/>
      </div>
      <p className="decision-why">{why}</p>
      <div className="decision-checks">
        {checks.map((check, index) => <span key={check} className="decision-check">{index === 0 ? <Shield size={10} color="var(--accent)"/> : <Check size={10} color={isBlocked ? "var(--err)" : isEscalated ? "var(--warn)" : "var(--accent)"}/>} {check}</span>)}
      </div>
      <div className="decision-actions">
        <button className="btn btn-ghost btn-sm" onClick={onOpenPolicies}>Change rule</button>
        {isEscalated && <button className="btn btn-accent btn-sm">Review payment</button>}
      </div>
    </div>
  );
}
...
function Sidebar({ screen, setScreen, nyraActive, setNyraActive, escalations, onLogout }) {
  const nav = [
    ...
    {id:"policies", icon:<Shield size={15}/>, label:"Your Controls"},
    {id:"deploy", icon:<Rocket size={15}/>, label:"Connect Nyra"},
...
function TopBar({ nyraActive, setNyraActive, screen, setScreen }) {
  const titles = {overview:"Today with Nyra",transactions:"Payment activity",ask:"Ask Nyra",policies:"Your controls",deploy:"Connect Nyra",settings:"Settings"};
  const subs = {overview:"See what Nyra is doing with your money",transactions:"Every payment, with its decision details",ask:"Your payment assistant",policies:"Set what Nyra can do on your behalf",deploy:"Use Nyra on your favourite chat apps",settings:"Preferences"};
  return (
...
function Overview({ nyraActive, setNyraActive, transactions, policy, escalations, setScreen }) {
  ...
  const attentionTx = transactions.find(tx => tx.status === "ESCALATED");
      ...
      <div className="page-row">
        <div className="page-main">
          <section className="agent-hero">
            <div className="agent-hero-top">
              <div>
                <div className="agent-kicker"><LiveDot/>{nyraActive ? "Nyra is active" : "Nyra is paused"}</div>
                <h2 className="agent-title">{nyraActive ? "Your money is handled—within your rules." : "Nyra is waiting for you to turn it back on."}</h2>
                <p className="agent-copy">{nyraActive ? "Nyra is monitoring your approved payment categories, trusted recipients, and spending limits. You stay in control of every exception." : "No new autonomous payments will be initiated while Nyra is paused."}</p>
              </div>
              <div style={{width:52,height:52,borderRadius:14,background:"rgba(168,230,61,0.1)",border:"1px solid rgba(168,230,61,0.24)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><NLogo size={29}/></div>
            </div>
            <div className="agent-next">
              <div className="agent-next-icon"><CheckCircle size={17} color="var(--accent)"/></div>
              <div style={{flex:1}}>
                <div className="agent-next-label">Next up</div>
                <div className="agent-next-title">IKEDC electricity · ₦4,500</div>
                <div className="agent-next-sub">Auto-pay Friday · trusted utility · within your limit</div>
              </div>
              <ChevronRight size={16} color="var(--t3)"/>
            </div>
            <div className="agent-actions">
              {attentionTx && <button className="btn btn-accent" onClick={()=>setScreen("transactions")}>Review {escalations} payment{escalations>1?"s":""} <ArrowRight size={14}/></button>}
              <button className="btn btn-ghost" onClick={()=>setScreen("ask")}><MessageSquare size={14}/>Ask Nyra</button>
              <button className="btn btn-outline" onClick={()=>setNyraActive(v=>!v)}>{nyraActive ? <><Pause size={14}/>Pause Nyra</> : <><Play size={14}/>Turn on Nyra</>}</button>
            </div>
          </section>
          ...
          <div className="stats-grid" style={{gridTemplateColumns:"repeat(3,minmax(0,1fr))"}}>
            <div className="stat-card">
              ...
              <div className="stat-value stat-mono" style={{cursor:"pointer"}} onClick={()=>setShowBal(s=>!s)}>{showBal?"₦1,250,000.00":"₦ ••••••••"}</div>
              <div style={{display:"flex",alignItems:"center",gap:6}}><span className="stat-sub">Across all accounts</span><button onClick={()=>setShowBal(s=>!s)} style={{background:"transparent",border:"none",cursor:"pointer",color:"var(--t3)",display:"flex",padding:0}}>{showBal?<Eye size={11}/>:<EyeOff size={11}/>}</button></div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Monthly spending</div>
              <div className="stat-value stat-mono">₦{spent.toLocaleString()}</div>
              <div className="stat-sub">{pct}% of your ₦{(policy.monthly/1000000).toFixed(0)}M limit</div>
              <div className="prog"><div className="prog-fill" style={{width:`${pct}%`}}/></div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Managed this month</div>
              <div className="stat-value">42</div>
              <div className="stat-sub">Payments checked by your rules</div>
              <div style={{height:40,marginTop:8}}><ResponsiveContainer width="100%" height="100%"><AreaChart data={SPEND_DATA} margin={{top:0,right:0,bottom:0,left:0}}><defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#A8E63D" stopOpacity={0.3}/><stop offset="100%" stopColor="#A8E63D" stopOpacity={0}/></linearGradient></defs><Area type="monotone" dataKey="v" stroke="#A8E63D" strokeWidth={1.5} fill="url(#ag)" dot={false}/></AreaChart></ResponsiveContainer></div>
            </div>
          </div>

          {attentionTx && (
            <section className="action-queue">
              <div className="sec-h" style={{marginBottom:9}}><span className="sec-title">Needs your decision</span><span style={{fontSize:11,color:"var(--warn)",fontWeight:700}}>1 item</span></div>
              <div className="queue-item" onClick={()=>setScreen("transactions")}>
                <div className="queue-icon"><AlertTriangle size={17} color="var(--warn)"/></div>
                <div style={{flex:1}}><div className="queue-eyebrow">Review required</div><div className="queue-title">{attentionTx.name} · ₦{attentionTx.amount.toLocaleString()}</div><div className="queue-copy">First-time recipient. Nyra is waiting for your approval.</div></div>
                <ChevronRight size={16} color="var(--warn)"/>
              </div>
            </section>
          )}

          <div className="card" style={{padding:"18px 20px"}}>
            <div className="sec-h"><span className="sec-title">Recent activity</span><button className="sec-link" onClick={()=>setScreen("transactions")}>View all</button></div>
            {transactions.slice(0,5).map(tx=>(
              <div key={tx.id} style={{marginBottom:2}}>
                <div className="tx-row" onClick={()=>setExpanded(expanded===tx.id?null:tx.id)}>
                  ...
                  <div><div className="tx-name">{tx.name}</div><div className="tx-cat">{tx.cat}</div></div>
                  <div className="tx-time">{tx.time}</div>
                  <div><div className="tx-amt">₦{tx.amount.toLocaleString()}</div></div>
                  <Chip status={tx.status}/>
                </div>
                {expanded===tx.id&&<DecisionReceipt tx={tx} onOpenPolicies={()=>setScreen("policies")}/>} 
              </div>
            ))}
          </div>
        </div>
        ...
        <aside className="page-right">
          <div className="card" style={{marginBottom:14}}>
            <div className="sec-h" style={{marginBottom:9}}><span className="sec-title">Your controls</span><button className="sec-link" onClick={()=>setScreen("policies")}>Edit</button></div>
            <p style={{fontSize:12,color:"var(--t2)",lineHeight:1.65,marginBottom:13}}>Nyra pays approved bills and trusted recipients up to ₦{policy.perTx.toLocaleString()}.</p>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {[["Auto-pay","Bills, utilities & entertainment","var(--accent)"],["Ask first","New recipients & transfers","var(--warn)"],["Never","Outside your monthly cap","var(--err)"]].map(([label,copy,color])=><div key={label} style={{display:"flex",gap:8,alignItems:"flex-start",padding:"8px 0",borderTop:"1px solid var(--border)"}}><div style={{width:7,height:7,borderRadius:"50%",background:color,marginTop:5,flexShrink:0}}/><div><div style={{fontSize:11,fontWeight:700,color:"var(--t1)"}}>{label}</div><div style={{fontSize:10,color:"var(--t2)",marginTop:2,lineHeight:1.4}}>{copy}</div></div></div>)}
            </div>
          </div>
          <div className="card">
            <div className="sec-h" style={{marginBottom:12}}><span className="sec-title">Ask Nyra</span></div>
            <p style={{fontSize:12,color:"var(--t2)",marginBottom:12,lineHeight:1.65}}>Tell Nyra what you need, or ask why a payment was handled a certain way.</p>
            <textarea className="ask-textarea" placeholder="e.g. Why was this payment blocked?" rows={3} readOnly onClick={()=>setScreen("ask")}/>
            <button className="ask-btn" style={{marginTop:8}} onClick={()=>setScreen("ask")}><Zap size={14} fill="#000"/>Ask Nyra</button>
          </div>
        </aside>
      </div>
    </div>
          ...
          if(msg.role==="pipeline"){
            ...
            return(
              <div key={msg.id} className="pipeline fa">
                <div className="pip-head"><Zap size={9} color="var(--accent)"/>What Nyra checked{p.status==="running"&&<span style={{marginLeft:"auto",color:"var(--accent)",fontSize:9}}>thinking…</span>}{p.status==="done"&&<span style={{marginLeft:"auto",color:"var(--accent)",fontSize:9}}>complete</span>}</div>
                {p.steps.map(s=>(
...
/* ── Transactions ────────────────────────────────────────────────────────── */
function Transactions({ transactions, setScreen }) {
  ...
  return (
    <div className="page">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:14,flexWrap:"wrap"}}>
        <div><div className="sec-title">Payment activity</div><p style={{fontSize:12,color:"var(--t2)",marginTop:4}}>Open any payment to see what Nyra checked and why it acted.</p></div>
        <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>{["all","approved","blocked","escalated"].map(f=><button key={f} onClick={()=>setFilter(f)} className="btn btn-sm" style={{background:filter===f?"var(--accent-dim)":"var(--s2)",color:filter===f?"var(--accent)":"var(--t2)",border:`1px solid ${filter===f?"rgba(168,230,61,0.2)":"var(--border)"}`,fontWeight:600}}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>)}</div>
      </div>
      <div className="card" style={{padding:"18px 20px"}}>
        {list.map((tx,i)=><div key={tx.id} style={{marginBottom:2}}><div className="tx-row fa" style={{animationDelay:`${i*25}ms`}} onClick={()=>setExpanded(expanded===tx.id?null:tx.id)}><div className="tx-ico" style={{background:tx.color}}><span style={{color:"#fff"}}>{tx.letter}</span></div><div><div className="tx-name">{tx.name}</div><div className="tx-cat">{tx.cat}</div></div><div className="tx-time">{tx.time}</div><div><div className="tx-amt">₦{tx.amount.toLocaleString()}</div></div><Chip status={tx.status}/></div>{expanded===tx.id&&<DecisionReceipt tx={tx} onOpenPolicies={()=>setScreen("policies")}/>}</div>)}
      </div>
    </div>
  );
}
...
function Policies({ policy, setPolicy }) {
  ...
  const modeOf = value => value === "never" ? "never" : value ? "auto" : "ask";
  const save=()=>{const {newR,...next}=d;setPolicy(next);setSaved(true);setTimeout(()=>setSaved(false),2000)};
  ...
  const setCategoryMode=(cat,mode)=>setD(p=>({...p,categories:{...p.categories,[cat]:mode==="auto"?true:mode==="never"?"never":false}}));
  const Section = ({title,sub,children}) => <div className="card" style={{marginBottom:14}}><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:15,color:"var(--t1)",marginBottom:sub?4:14,letterSpacing:"-0.01em"}}>{title}</div>{sub&&<p style={{fontSize:11,color:"var(--t2)",lineHeight:1.55,marginBottom:14}}>{sub}</p>}{children}</div>;

  return (
    <div className="page" style={{maxWidth:720}}>
      {d.emergency&&<div style={{display:"flex",gap:10,padding:"12px 16px",background:"var(--err-dim)",border:"1px solid rgba(255,68,68,0.2)",borderRadius:10,marginBottom:14,alignItems:"center"}}><AlertTriangle size={15} color="var(--err)"/><span style={{fontSize:13,fontWeight:600,color:"var(--err)",flex:1}}>Emergency pause is active. Nyra cannot start new payments.</span><button className="btn btn-danger btn-sm" onClick={()=>setD(p=>({...p,emergency:false}))}>Resume</button></div>}

      <section className="permission-summary">
        <h2>Nyra’s payment permissions</h2>
        <p>These rules decide what Nyra can handle on your behalf. Any exception is brought back to you before money moves.</p>
        <div className="permission-grid">
          <div className="permission-pill auto"><strong>Auto-pay</strong><span>Bills, utilities, entertainment, and trusted recipients within your limit.</span></div>
          <div className="permission-pill ask"><strong>Ask first</strong><span>New recipients, transfers, or categories you have not approved.</span></div>
          <div className="permission-pill never"><strong>Never</strong><span>Anything outside your monthly cap, allowed hours, or safety rules.</span></div>
        </div>
      </section>

      <Section title="How much can Nyra spend?" sub="Nyra will stop and ask you whenever a payment exceeds either amount.">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div className="inp-wrap"><label className="inp-label">Per-payment limit (₦)</label><input className="inp" type="number" value={d.perTx} onChange={e=>setD(p=>({...p,perTx:e.target.value}))}/></div>
          <div className="inp-wrap"><label className="inp-label">Monthly limit (₦)</label><input className="inp" type="number" value={d.monthly} onChange={e=>setD(p=>({...p,monthly:e.target.value}))}/></div>
        </div>
      </Section>

      <Section title="Who can Nyra pay automatically?" sub="Trusted recipients can be paid automatically if all of your other rules are met.">
        <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:12}}>{d.whitelist.map(r=><span key={r} className="pchip">{r.toUpperCase()}<button aria-label={`Remove ${r}`} onClick={()=>rmR(r)}><X size={10}/></button></span>)}</div>
        <div style={{display:"flex",gap:8}}><input className="inp" placeholder="Add recipient (e.g. dstv)" value={d.newR} onChange={e=>setD(p=>({...p,newR:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addR()} style={{flex:1}}/><button className="btn btn-ghost btn-sm" aria-label="Add trusted recipient" onClick={addR}><Plus size={14}/></button></div>
      </Section>

      <Section title="What can Nyra handle?" sub="Choose a permission level for each payment type. You can change these choices at any time.">
        {Object.entries(d.categories).map(([cat,value])=>{
          const mode=modeOf(value);
          const description=mode==="auto"?"Nyra can pay this within your rules.":mode==="ask"?"Nyra will ask before paying this.":"Nyra will not pay this category.";
          return <div key={cat} className="pol-row" style={{gap:16}}><div style={{minWidth:0}}><div className="pol-title" style={{textTransform:"capitalize"}}>{cat}</div><div className="pol-sub">{description}</div></div><div className="segmented">{[["auto","Auto-pay"],["ask","Ask first"],["never","Never"]].map(([key,label])=><button key={key} className={`seg-btn ${key} ${mode===key?"on":""}`} onClick={()=>setCategoryMode(cat,key)}>{label}</button>)}</div></div>
        })}
      </Section>

      <Section title="When may Nyra act?" sub="Outside these hours, Nyra will wait for you to review the payment.">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}><div className="inp-wrap"><label className="inp-label">From (24h)</label><input className="inp" type="number" min={0} max={23} value={d.startH} onChange={e=>setD(p=>({...p,startH:e.target.value}))}/></div><div className="inp-wrap"><label className="inp-label">Until (24h)</label><input className="inp" type="number" min={0} max={23} value={d.endH} onChange={e=>setD(p=>({...p,endH:e.target.value}))}/></div></div>
      </Section>

      <Section title="Safety controls">
        <div className="pol-row"><div><div className="pol-title">Always ask for first-time recipients</div><div className="pol-sub">Nyra will never send money to a new contact without your approval.</div></div><Toggle checked={d.firstTime} onChange={v=>setD(p=>({...p,firstTime:v}))}/></div>
        <div className="pol-row"><div><div className="pol-title" style={{color:"var(--err)"}}>Emergency pause</div><div className="pol-sub">Immediately stop Nyra from starting new autonomous payments.</div></div><Toggle checked={d.emergency} onChange={v=>setD(p=>({...p,emergency:v}))}/></div>
      </Section>
      <button className="btn btn-accent btn-full btn-lg" onClick={save}>{saved?<><Check size={16}/>Permissions saved</>:"Save permissions"}</button>
    </div>
  );
}
    ...
    <div className="page" style={{maxWidth:680}}>
      <div className="card" style={{marginBottom:14,background:"var(--accent-dim)",borderColor:"rgba(168,230,61,0.15)"}}>
        <div style={{display:"flex",gap:10}}>
          ...
          <div><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:15,color:"var(--t1)",marginBottom:4}}>Use Nyra beyond this app</div><div style={{fontSize:13,color:"var(--t2)",lineHeight:1.65}}>Connect Nyra to the chat apps you already use. Your payment rules and decision history stay consistent everywhere.</div></div>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
...
export default function NyraApp() {
  ...
  const screenMap = {
    ...
    transactions: <Transactions transactions={transactions} setScreen={setScreen}/>,
  ...
  const mobileNav = [
    ...
    {id:"ask",icon:<MessageSquare size={20}/>,label:"Nyra"},
    {id:"policies",icon:<Shield size={20}/>,label:"Controls"},
  ];
